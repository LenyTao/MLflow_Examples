...
def run(config: ConfigFile):
    ### Указываем хост где поднят mlflow
    ...
    with mlflow.start_run():
        ### Далее начинается простенькое машинное обучение, оно не относится к mlflow
        ### Поэтому можно не вникать в него, для mlflow нам важен лишь его результат
        ### Оно закончится комментарием с полосой, таким же как на следующей строчке
        # --------------------------------------------------------------------------------------------------------------
        ...
        raw_covid_df = \
            spark \
                ...
                .option('header', True) \
                .csv(
                config.spark_ml.training_data_path
            )

        def convert_date_to_fact(date: str):
            return 0 if date.__contains__("9999-99-99") else 1

        convert_date_to_fact_udf = udf(lambda date: convert_date_to_fact(date), IntegerType())

        converted_df = raw_covid_df.withColumn("death",
                                               convert_date_to_fact_udf(col("date_died"))
                                               )

        clear_covid_df = converted_df.drop("id", "entry_date", "date_symptoms", "date_died")
        ...
        ### На основе него были созданы и оценены Regression Model и Classifier Model модели,
        ### Теперь мы можем снова вернутся к MlFlow

        ### Нам необходимо создать временную директорию для создания различных артефактов с необходимой информацией,
        ### которые в дальнейшем будут перемещены в MlFlow
        ...
        ### Сохраним только что обученные модели в необходимое место на локалке для быстрого использования
